...
class DetectorTCPClient:
    ...
    def __init__(self, host: str = '127.0.0.1', json_port: int = 1234, 
                 data_port: int = 1235, live_port: int = 1236) -> None:
        self.host: str = host
        self.json_port: int = json_port
        self.data_port: int = data_port
        self.live_port: int = live_port
        self.json_socket: Optional[socket.socket] = None
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            await asyncio.get_event_loop().sock_connect(self.json_socket, (self.host, self.json_port))
            
            self.data_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.data_socket.setblocking(False)  # Make socket non-blocking
            await asyncio.get_event_loop().sock_connect(self.data_socket, (self.host, self.data_port))
            ...
            await asyncio.get_event_loop().sock_connect(self.live_socket, (self.host, self.live_port))
    ...
    async def send_command(self, cmd_type: str, *, action: Optional[str] = None, 
                          parameter: Optional[str] = None, value: Optional[str] = None) -> Optional[Dict[str, Any]]:
                ...
                while True:
                    try:
                        ...
                        data: bytes = await asyncio.wait_for(
                            loop.sock_recv(self.json_socket, 1), 
                            timeout=5.0
                        )
    ...
    async def get_all_parameters(self) -> Optional[Dict[str, Any]]:
        """Get all parameters from detector."""
        return await self.send_command("GET", parameter="*")
    
    async def get_parameter(self, param_name: str) -> Optional[Dict[str, Any]]:
        """Get specific parameter from detector."""
        return await self.send_command("GET", parameter=param_name)
    
    async def set_parameter(self, param_name: str, value: Any) -> Optional[Dict[str, Any]]:
...
class DetectorIOC(PVGroup):
    ...
    async def _param_write(self, instance: PvpropertyData, value: Any) -> Any:
        ...
        result = next((item for item in response["values"] if item["name"] == param_name), None)
        if not result:
            await self.last_error.write(f"Failed to get new value of {param_name}")
            return None
        actual_value = result["value"]
        if actual_value != value:
            await self.last_error.write(f"Failed to set {param_name} to {value}, was set to {actual_value} instead.")
        return actual_value
    ...
    sync = pvproperty(value="ON", name="SYS:SYNC", dtype=ChannelType.ENUM, enum_strings=("OFF", "ON"), record="bi")
    ...
    pass_energy = pvproperty(put=_param_write, name="PASS_ENERGY", dtype=ChannelType.ENUM, enum_strings=("PE001",
                "PE002",
                "PE005",
                "PE010",
                "PE020",
                "PE050"))
    lens_mode = pvproperty(put=_param_write, name="LENS_MODE", dtype=ChannelType.ENUM, enum_strings=(
        "L4Ang0d6",
        "L4Ang0d8",
        "L4Ang1d6",
        "L4Ang3d9",
    ...
    acq_mode = pvproperty(put=_param_write, name="ACQ_MODE", dtype=ChannelType.ENUM, enum_strings=("Fixed", "Swept", "Dither"))
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._update_listener_task: Optional[asyncio.Task] = None
        ...
        if instance.value == "ON":
            try:
                ...
                if response and 'values' in response:
                    ...
                    await self.last_error.write("Failed to sync parameters - no response or invalid response")
    ...
    async def _update_pvs_from_response(self, response: Dict[str, Any], async_lib: Any) -> None:
        ...
        try:
            ...
            for item in values:
                if item["name"] in self._param_names_to_pvs:
                    write_tasks.append(self._param_names_to_pvs[item["name"]].write(item["value"]))
    ...
    async def acquire(self, instance: Any, value: int) -> int:
        ...
        if value > 0:
            ...
            response: Optional[Dict[str, Any]] = await self.tcp_client.send_command("ACTION", action="STOP")
            if response:
    ...
    async def monitor_on(self, instance: Any, value: int) -> int:
        """Turn monitoring on."""
        if value == 1:
            response: Optional[Dict[str, Any]] = await self.tcp_client.send_command("ACTION", action="MONITOR_ON")
            if response:
    ...
    async def monitor_off(self, instance: Any, value: int) -> int:
        """Turn monitoring off."""
        if value == 1:
            response: Optional[Dict[str, Any]] = await self.tcp_client.send_command("ACTION", action="MONITOR_OFF")
            if response:
    ...
    async def detector_off(self, instance: Any, value: int) -> int:
        """Turn detector off."""
        if value == 1:
            response: Optional[Dict[str, Any]] = await self.tcp_client.send_command("ACTION", action="DET_OFF")
            if response:
    ...
    async def get_image(self, instance: Any, value: int) -> int:
        ...
        if value == 1:
            response: Optional[Dict[str, Any]] = await self.tcp_client.send_command("ACTION", action="GET_IMAGE")
            if response:
    ...
    async def get_stats(self, instance: Any, value: int) -> int:
        ...
        if value == 1:
            response: Optional[Dict[str, Any]] = await self.tcp_client.send_command("ACTION", action="GET_ACQ_STATS")
            if response:
