...
def SET(par, val):
    return '{{"cmd":"SET","id":{},"values":{{"{}":"{}"}}}}'.format(
        random.randrange(100), par, val
    )
...
def MON_ON():
    return '{{"cmd":"ACTION","id":{},"values":"MONITOR_ON"}}'.format(
        random.randrange(100)
    )


def MON_OFF():
    return '{{"cmd":"ACTION","id":{},"values":"MONITOR_OFF"}}'.format(
        random.randrange(100)
    )
...
def REPLACE_ADD(on=True):
    return (
        f'{{"cmd":"SET","id":{random.randrange(100)},"values":{{"replaceAdd":"{on}"}}}}'
    )


def GET_IMAGE():
    return '{{"cmd":"ACTION","id:{},"values":"GET_IMAGE"}}'.format(
        random.randrange(100)
    )


def GET_ACQ_STATS():
    return '{{"cmd":"ACTION","id:{},"values":"GET_ACQ_STATS"}}'.format(
        random.randrange(100)
    )
...
def connect_to_server():
    ...
    assert (
        json_socket is not None and data_socket is not None and live_socket is not None
    )
...
def get_live_img(socket):
    try:
        ...
        _ = int.from_bytes(header[8:12], byteorder="big", signed=False)
        _ = int.from_bytes(header[12:16], byteorder="big", signed=False)
        length = int.from_bytes(header[16:20], byteorder="big", signed=False)

        if length:
            # print(f"Got live img of length {length}")
            _ = socket.recv(length)
...
def input_task(json_socket, data_socket, live_socket, data_queue, live_queue, lock):
    ...
    while True:
        ...
        if s == "":
            ...
            print(
                "Test client for MBSEA project. Sends commands over socket to MBSEA server. "
...
def main():
    ...
    data_thr_q = mp.Queue()
    data_recv_thr = mp.Process(
        target=recv_data, args=(json_socket, data_socket, data_thr_q, lock)
    )
    data_recv_thr.start()

    live_thr_q = mp.Queue()
    live_data_thr = mp.Process(
        target=live_data, args=(json_socket, live_socket, live_thr_q, lock)
    )
    live_data_thr.start()
