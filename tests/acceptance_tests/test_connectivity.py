...
class TestBasicConnectivity:
    ...
    def test_device_connection(self, analyzer_device):
        ...
        assert device.connection_status.get() == 1, (
            "IOC should be connected to hardware"
        )
    ...
    def test_sync_functionality(self, analyzer_device):
        ...
        assert last_sync_after != last_sync_before, (
            "Sync should have updated last_sync timestamp"
        )
...
class TestStatusMonitoring:
    ...
    def test_acquisition_status_consistency(self, detector_in_standby):
        ...
        if state == "STANDBY":
            assert device.acquisition_status.get() == 0, (
                "Status should be 0 when in STANDBY"
            )
    ...
    def test_connection_status_stability(self, analyzer_device):
        ...
        for i in range(5):
            ...
            assert status == 1, (
                f"Connection status should remain 1, got {status} on check {i}"
            )
            time.sleep(0.5)
    ...
    def test_parameter_consistency(self, analyzer_device):
        ...
        for param in stable_params:
            ...
            assert all(v == values[0] for v in values), (
                f"Parameter {param.name} should be stable"
            )
