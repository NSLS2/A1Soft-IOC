...
class TestFileCapture:
    ...
    def test_enable_file_capture(self, detector_in_standby, test_output_dir):
        ...
        assert device.file_capture.get(as_string=True) == "Off", "File capture should start as Off"
        ...
        assert "started" in file_status.lower(), f"File status should indicate started: {file_status}"
    ...
    def test_disable_file_capture(self, detector_in_standby, test_output_dir):
        ...
        # Check file status
        file_status = device.file_status.get()
        assert "stopped" in file_status.lower(), f"File status should indicate stopped: {file_status}"
    ...
    def test_file_capture_error_conditions(self, detector_in_standby, test_output_dir):
        ...
        device.file_path.set("")
        device.file_name.set("test.nxs")
        ...
        device.file_name.set("")
        
        with pytest.raises(Exception):
            device.file_capture.set("On")
        
        # Test with invalid file extension
        device.file_name.set("test.txt")
...
class TestFileWriting:
    ...
    def test_acquisition_with_file_capture(self, detector_in_standby, test_output_dir):
        ...
        try:
            ...
            assert final_num_processed >= initial_num_processed, "Should have processed some data"
...
class TestFileCounters:
    ...
    def test_num_processed_during_acquisition(self, detector_in_standby, test_output_dir):
        ...
        try:
            ...
            while (time.time() - start_time) < monitoring_time and device.state.get() == "RUNNING":
                current_processed = device.num_processed.get()
                assert current_processed >= initial_processed, "num_processed should not decrease"
                time.sleep(0.2)
    ...
    def test_file_status_messages(self, detector_in_standby, test_output_dir):
        ...
        assert active_status != initial_status, "Status should change when capture starts"
...
class TestFilePathHandling:
    ...
    def test_path_and_name_validation(self, detector_in_standby, test_output_dir):
        ...
        for name in valid_names:
            device.file_path.set(valid_path)
            device.file_name.set(name)
